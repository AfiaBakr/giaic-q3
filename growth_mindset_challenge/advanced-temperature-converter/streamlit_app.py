...
def local_css(mode):
    if mode == "light":
        ...
        <style>
        .stApp {
            background: linear-gradient(135deg,rgb(123, 65, 170) 0%,rgb(158, 109, 185) 100%);
            color: #ecf0f1;
        }
        .main .block-container {
...
with st.expander("Temperature Conversion Quiz"):
    ...
    if st.session_state.current_question < len(questions):
        ...
        answer = st.radio("Select your answer:", question["options"], key=f"q{str(st.session_state.current_question)}")
